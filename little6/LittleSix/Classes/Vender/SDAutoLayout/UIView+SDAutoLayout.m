...
- (SpaceToSuperView)spaceToSuperView {
    ...
    if (!_spaceToSuperView) {
        _spaceToSuperView = ^(UIEdgeInsets insets) {
            ...
            if (superView) {
                weakSelf.needsAutoResizeView.sd_layout.leftSpaceToView(superView, insets.left)
...
- (void)setSd_bottomViewBottomMargin:(CGFloat)sd_bottomViewBottomMargin {
    objc_setAssociatedObject(self, @selector(sd_bottomViewBottomMargin), @(sd_bottomViewBottomMargin),
...
- (void)setDidFinishAutoLayoutBlock:(void (^)(CGRect))didFinishAutoLayoutBlock {
    objc_setAssociatedObject(self, @selector(didFinishAutoLayoutBlock), didFinishAutoLayoutBlock,
...
- (void)setSd_cornerRadiusFromWidthRatio:(NSNumber *)sd_cornerRadiusFromWidthRatio {
    objc_setAssociatedObject(self, @selector(sd_cornerRadiusFromWidthRatio), sd_cornerRadiusFromWidthRatio,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)setSd_cornerRadiusFromHeightRatio:(NSNumber *)sd_cornerRadiusFromHeightRatio {
    objc_setAssociatedObject(self, @selector(sd_cornerRadiusFromHeightRatio), sd_cornerRadiusFromHeightRatio,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)setSd_equalWidthSubviews:(NSArray *)sd_equalWidthSubviews {
    objc_setAssociatedObject(self, @selector(sd_equalWidthSubviews), sd_equalWidthSubviews,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)setIsAttributedContent:(BOOL)isAttributedContent {
    objc_setAssociatedObject(self, @selector(isAttributedContent), @(isAttributedContent),
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
    ...
    if (self.tag == kSDModelCellTag && [self isKindOfClass:NSClassFromString(@"UITableViewCellContentView")]) {
        ...
        if ([cell isKindOfClass:[UITableViewCell class]]) {
            ...
            cell.autoHeight = height + cell.sd_bottomViewBottomMargin;
               ...
               (self.sd_bottomViewsArray.count || self.sd_rightViewsArray.count)) {
...
- (void)sd_resizeWithModel:(SDAutoLayoutModel *)model {
            ...
            if (label.text.length) {
                if (!label.isAttributedContent) {
                    CGRect rect = [label.text
                        boundingRectWithSize:CGSizeMake(width, label.height)
    ...
    if (view.autoHeightRatioValue && view.width > 0 &&
        (model.bottomEqualToView || model.bottomSpaceToView)) { // 底部布局前提设置
        if ([view.autoHeightRatioValue floatValue] > 0) {
            ...
            if ([view isKindOfClass:[UILabel class]]) {
                ...
                if (label.text.length) {
                    if (!label.isAttributedContent) {
                        CGRect rect = [label.text
                            boundingRectWithSize:CGSizeMake(label.width, MAXFLOAT)
                                         options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading
                                      attributes:@{
                                          NSFontAttributeName : label.font
                                      }
    ...
    } else if (view.sd_cornerRadiusFromWidthRatio &&
               (cornerRadius != [view.sd_cornerRadiusFromWidthRatio floatValue] * view.width)) {
        newCornerRadius = view.width * [view.sd_cornerRadiusFromWidthRatio floatValue];
    } else if (view.sd_cornerRadiusFromHeightRatio &&
               (cornerRadius != view.height * [view.sd_cornerRadiusFromHeightRatio floatValue])) {

...
- (instancetype)init {
    ...
    UIBezierPath *path =
        [UIBezierPath bezierPathWithRoundedRect:CGRectInset(_progressLayer.bounds, 7, 7) cornerRadius:(40 / 2 - 7)];
...
- (void)_initAddtionView {

    if (self.isSingleView) return;
    ...
    moreButton.backgroundColor = HEXRGB(0x999999);
    [moreButton setImage:[UIImage imageNamed:@"navi_chat_more"] forState:UIControlStateNormal];
...
- (void)updateInstructionLabel {
    NSString *text = [NSString stringWithFormat:@"%ld/%ld", self.currentPage + 1, self.groupItems.count];
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    [_scrollView
        scrollRectToVisible:CGRectMake(_scrollView.width * _pager.currentPage, 0, _scrollView.width, _scrollView.height)
                   animated:NO];
    [self scrollViewDidScroll:_scrollView];

    [UIView setAnimationsEnabled:YES];
    //    _fromNavigationBarHidden = [UIApplication sharedApplication].statusBarHidden;
    _fromNavigationBarHidden = NO;

    //    [[UIApplication sharedApplication] setStatusBarHidden:YES withAnimation:animated ? UIStatusBarAnimationFade :
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    [[UIApplication sharedApplication]
        setStatusBarHidden:_fromNavigationBarHidden
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            [UIView animateWithDuration:0.1
                             ...
                             animations:^{
                                 ...
                                 //自定义控件的alpha
                                 self.instructionLabel.alpha = alpha;
...
- (void)moreButtonAction:(UIButton *)button {
    ...
    ModelComfirm *item1 = [ModelComfirm comfirmModelWith:@"举报" titleColor:HEXRGB(0x666666) fontSize:16];
    ModelComfirm *item2 = [ModelComfirm comfirmModelWith:@"点赞" titleColor:HEXRGB(0x666666) fontSize:16];
    ModelComfirm *item3 = [ModelComfirm comfirmModelWith:@"保存图片" titleColor:HEXRGB(0x666666) fontSize:16];
        ...
        cancelItemWith:cancelItem
           ...
           actionBlock:^(ComfirmView *view, NSInteger index) {
               ...
               if (index == 0) {
                   ...
                   UIImage *image =
                       [[YYImageCache sharedCache] getImageForKey:item.largeImageURL.absoluteString];
                   if (image) {
                       UIImageWriteToSavedPhotosAlbum(
                           image, self, @selector(saveImage:didFinishWriteWithError:contextInfo:), nil);
                   } else {
                       NSError *error = [NSError new];
                       [self saveImage:nil didFinishWriteWithError:error contextInfo:nil];
                   }
               }
...
- (void)saveImage:(UIImage *)image didFinishWriteWithError:(NSError *)error contextInfo:(void *)contextInfo {

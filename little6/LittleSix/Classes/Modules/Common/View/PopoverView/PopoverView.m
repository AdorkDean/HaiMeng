...
#define kArrowOffset 12

static float const kPopoverViewMargin = 6.f; ///< 边距
...
- (void)showToPoint:(CGPoint)toPoint {
    ...
    [maskPath addArcWithCenter:CGPointMake(currentW - cornerRadius, maskTop + cornerRadius)
                        radius:cornerRadius
                    startAngle:DegreesToRadian(270)
                      endAngle:DegreesToRadian(0)
                     clockwise:YES];
    // 右下圆角
    ...
    [maskPath addArcWithCenter:CGPointMake(cornerRadius, maskBottom - cornerRadius)
                        radius:cornerRadius
                    startAngle:DegreesToRadian(90)
                      endAngle:DegreesToRadian(180)
                     clockwise:YES];
    [maskPath closePath];
    // 截取圆角和箭头
...
- (void)showToView:(UIView *)pointView withActions:(NSArray<PopoverAction *> *)actions {
    ...
    if (pointViewUpLength > pointViewDownLength) {
        toPoint.y = pointViewUpLength - kArrowOffset;
    }
    // 弹窗在 pointView 底部
    else {
        toPoint.y = CGRectGetMaxY(pointViewRect) + kArrowOffset;
    }

...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    if (self.activeLink) {
        ...
        if (self.delegate&&[self.delegate respondsToSelector:@selector(didAllClickLinkLabel:)]) {
            
            [self.delegate didAllClickLinkLabel:self];
        }
...
- (void)longPressGestureDidFire:(UILongPressGestureRecognizer *)sender {
    ...
    if (sender.state==UIGestureRecognizerStateBegan) {
        ...
        if (link) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(didLongAllPressLinkLabel:)]) {
                
                [self.delegate didLongAllPressLinkLabel:self];
            }
        }
    }
}
